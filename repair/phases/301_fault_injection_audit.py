#!/usr/bin/env python3
"""Phase 3.1: Fault Injection Audit. Proves the holes closed by 201-203 are actually closed."""
import os, sys
sys.path.insert(0, os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "00_harness"))
import safe_edit as se

C_CODE = r'''#include <lancius.h>
...
def main():
    print("[301] fault injection audit")
    se.create_file("examples/audit_fault_injection.c", C_CODE, "FAULT INJECTION AUDIT")

    # Add to makefile
    mk_path = se.p("makefile")
    mk = se.read(mk_path)
    if "audit_fault_injection" not in mk:
        # Add to 'all' target
        mk = mk.replace("audit_fp32_path\n", "audit_fp32_path audit_fault_injection\n")
        # Add to 'check' target
        mk = mk.replace("./audit_fp32_path\n", "./audit_fp32_path\n\t./audit_fault_injection\n")
        # Add build rule at the end
        mk += "\naudit_fault_injection: examples/audit_fault_injection.c liblancius.a\n\t$(CC) $(CFLAGS) -o $@ $< liblancius.a $(LDFLAGS) -fopenmp -lpthread\n"

        se.backup(mk_path)
        se.write(mk_path, mk)
        print("  [makefile] added audit_fault_injection")
